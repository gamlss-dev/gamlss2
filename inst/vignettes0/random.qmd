...
```{r}
m2 <- gamlss2(y ~ x1 + x2 + re(random=~x1|cluster), 
              sigma.formula = ~ x2,
              data=toydata1, family=GA)
...
First fit a random intercepts model using `lme()`. The model is
...
The fitted model gives estimates $\hat\beta_0 = 24.02$,  $\hat\beta_1 = 0.660$, $\hat\sigma_a = 2.115$ and
$\hat\sigma = 1.432$.
...
Extract fitted random intercept special model term:
```{r}
...
The fitted model gives estimates $\hat\beta_0 = 24.02$,  $\hat\beta_1 = 0.660$, $\hat\sigma_a = 2.072$ and $\hat\sigma = 0.486$. 
The estimate  $\hat\sigma$ is seriously negatively biased, because it is *not* a REML estimate.

For this simple random intercepts model, the estimate of $\sigma$ can be adjusted by multiplying by the local LME residual given by 1.135 giving: 

::: {.callout-warning}
`Bob` This is wrong. Where does 1.135 come from?
::: 

adjusted $\hat\sigma = 0.486\times1.135 = 0.552$, which is very close to the `lme` estimate of $\sigma$. 
...
We fit  model (@eq-toydata2) with random intercepts in the $\mu$ and $\sigma$ models:



```{r}
m3 <- gamlss2(y ~ x1 + x2 + re(random=~1|cluster), 
