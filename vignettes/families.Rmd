...
Note that all family objects of the _gamlss.dist_ package can be used for modeling. However, for
users wanting to specify their own (new) distribution model, this document

Family objects in the _gamlss2_ package play an essential role in defining the models used for
...
* The link functions that map the parameters to the linear predictor.
* Functions for the density, log-likelihood, and their derivatives.

This document provides an overview of how to construct and use family objects within _gamlss2_.
...
is named `"hess"`. An example of setting up first and second order derivatives for the normal is
...
In the following example, we will create a **family object** for the Kumaraswamy distribution
...
The **Kumaraswamy family** object in _gamlss2_ is flexible, allowing the user to specify
...
Family objects in the _gamlss2_ package play a central role in defining the distributional model
used for regression. The **Kumaraswamy family** is an example of a distributional family that
can be used for modeling continuous data on the \((0, 1)\) interval. With its customizable link
functions and simple structure, it is a useful tool for distributional regression in various
settings.
