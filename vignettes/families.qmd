...
distribution parameter. Second-order derivatives must be provided in a list named `"hessian"`. Each 
function in `"hessian"` must return the **negative (expected)** second derivative with respect to the 
predictor.

The following code illustrates how to provide both `"score"` and `"hessian"` functions for
...
can also be specified. These should be added to the `"hessian"` list and named using the
...
* `quantile(par, p, ...)`: Quantile function with probability vector `p`.
* `support(par, ...)`: Lower and upper response support as a numeric matrix
  with columns `min` and `max`, one row per parameter combination. A constant
  support can be supplied as `c(min, max)`. If omitted, support is inferred
  from `quantile(par, 0)` and `quantile(par, 1)` when available. Without
  either, discrete families are assumed to be count distributions on the
  nonnegative integers, while continuous families obtain a
  parameter-dependent support by probing the log-density. The continuous
  fallback is heuristic; shifted or truncated counts and unusual,
  disconnected, or numerically extreme continuous supports should be supplied
  explicitly.
* `random(par, n, ...)`: Random number generation with number of samples `n`.
...
Note that the `cdf()` function is needed for computing the quantile residuals.
For a family with a `pdf()` and support, `complete_family()` constructs a
missing `cdf()` by numerically integrating a continuous density or summing a
discrete probability mass function over its integer support. Analytical CDFs
are left unchanged. If `quantile()` is missing but a CDF and support are
available, `complete_family()` constructs it by numerical inversion. For a
discrete family, this returns the smallest integer whose CDF reaches the
requested probability. Analytical quantile functions are left unchanged.
Missing `mean()` and `variance()` functions are computed from the density by
adaptive integration for continuous families and batched PMF summation for
discrete families. A missing `random()` function is generated by applying the
quantile function to uniform random probabilities. Existing analytical
functions are always retained.

For faster estimation, it can also be useful to provide an optimized
update function

* `update(par, y, eta, which)`

where `eta` denotes the predictor and `which` is a character string 
identifying the parameter to be updated, as specified in the family's
"names" component.

The function must return a named list of the form `list(eta, weights)`,
where `eta` is the updated predictor after one Newton–Raphson or
Fisher-scoring iteration, and `weights` contains the corresponding working 
weights used in subsequent fitting steps, such as IWLS updates.

The main benefit of implementing this function arises
when an optimized C implementation is provided for updating the predictor 
within the package's default backfitting or MCMC algorithms.
...
Normal <- function(...) {
  ...
  fam$update <- function(par, y, eta, which) {
    score <- fam$score[[which]](par, y)
    hessian <- fam$hessian[[which]](par, y)
...
adapting the `score` and `hessian` functions to work on the linear predictor scale.
...
Note that the `score` and `hessian` elements are omitted and approximated
...
## estimate models
m1 <- gamlss2(f, data = d, family = Normal(mu.link = "identity"))
m2 <- gamlss2(f, data = d, family = Normal(mu.link = softplus))
```

The fitted quantiles are visualized with
...
plot(fev1 ~ age, data = d, pch = 19,
  col = rgb(0.1, 0.1, 0.1, alpha = 0.3))

q1 <- quantile(m1, probs = c(0.025, 0.5, 0.975))
q2 <- quantile(m2, probs = c(0.025, 0.5, 0.975))

i <- order(d$age)
matlines(d$age[i], q1[i, ], col = 2, lwd = 5)
matlines(d$age[i], q2[i, ], col = 4, lwd = 2)
