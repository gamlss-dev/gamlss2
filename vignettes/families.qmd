...
* `support(par, ...)`: Lower and upper response support as a numeric matrix
  ...
  from `quantile(par, 0)` and `quantile(par, 1)` when available. Otherwise,
  continuous families obtain a parameter-dependent support by probing the
  log-density. This last fallback is heuristic, so unusual, disconnected, or
  numerically extreme supports should be supplied explicitly.
...
For a continuous family, `complete_family()` constructs a missing `cdf()` by
numerically integrating `pdf()` over the family support. Analytical CDFs are
left unchanged. This fallback is not currently generated for discrete families.
