...
* `p(y, par, ...)`: Cumulative distribution function.
* `q(p, par, ...)`: Quantile function with probability vector `p`.
* `r(n, par, ...)`: Random number generation.
...
m2 <- gamlss2(f, data = abdom, family = Normal(mu.link = softplus(a = 5)))
```
