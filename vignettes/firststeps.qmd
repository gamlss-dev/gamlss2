...
```
```{r}
f1 <- Tmax ~ ga(~ ti(year, k = 10) + ti(yday, bs = "cc", k = 10) +
  ti(year, yday, bs = c("cr", "cc"), k = c(5, 5)))
m1 <- gamlss(f1, family = SEP,
  data = MUC[, c("Tmax", "year", "yday")])
```

This setup requires loading the `gamlss.add` package to access `mgcv`-based
smooth terms. Estimation takes `r m1$iter` iterations of the backfitting algorithm (without full convergence) 
and about 62 seconds on a 64-bit Linux system. Moreover, `gamlss()` requires that the
input data contains no `NA` values. In `gamlss2` the model can be
specified directly, following `mgcv` syntax

```{r}
f2 <- Tmax ~ ti(year, k = 10) + ti(yday, bs = "cc", k = 10) +
  ti(year, yday, bs = c("cr", "cc"), k = c(5, 5))
m2 <- gamlss2(f2, family = SEP, data = MUC)
```

This model converges in `r m2$iterations` iterations and requires only about
`r round(m2$elapsed)` seconds of computation time, 
...
```{r}
f3 <- Tmax ~ ti(year, k = 10) + ti(yday, bs = "cc", k = 10) +
  ti(year, yday, bs = c("cr", "cc"), k = c(5, 5)) | . | . | .
m3 <- gamlss2(f3, family = SEP, data = MUC)
```

This model converges in `r m3$iterations` iterations in about
`r round(m3$elapsed)` seconds. After estimation, results can be
...
showing good model calibration. Next, we compute the probability of a heat day for 2025.
First, the `procast()` function from `topmodels` predicts the fitted distributions

```{r}
nd <- data.frame("year" = 2025, "yday" = 0:365)
pf <- procast(m3, newdata = nd, drop = TRUE)
```
...
Finally, estimated quantiles can be computed, e.g., with

```{r}
pq <- quantile(pf,
  probs = c(0.01, 0.025, 0.1, 0.5, 0.9, 0.975, 0.99))
print(head(pq))
```
...
#| fig-align: center
par(mar = c(4, 4, 1, 1))
plot(Tmax ~ yday, data = MUC,
  pch = 16, col = adjustcolor(1, 0.1))
matlines(nd$yday, pq, lwd = 2, lty = 1, col = 4)
```
