...
Save the data with only two variables and fit `random()` in `gamlss`:

```{r}
da <- da1<- citdatasample |>
  select(n_citation, venue) 
```
...
data_str(da)
data_dim(da)
da <-data_few2fac(da, 17)
  m1 <- gamlss(n_citation~random(venue), data=da, family=NO)
```

Fitted a normal distribution is probably not a good idea so I repeat the data using the negative binomial.

```{r}
g1  <- gamlss(n_citation~re(random=~1|venue), data=da1,family=NBI)
wp(g1)
```

This worm plot is not particularly good, so we will try to fit different types of distributions. First, we fit the negative binomial type two;

```{r}
g1  <- gamlss(n_citation~re(random=~1|venue), data=da1, family=NBII)
```

Then we fit the `PIG` distribution;

```{r}
g2  <- gamlss(n_citation~re(random=~1|venue), data=da1, family=PIG)
wp(g2, ylim.all=.5)
```

Finally, we fit the `SICHEL` distribution;

```{r}
g3  <- gamlss(n_citation~re(random=~1|venue), data=da1, family=SICHEL, n.cyc=50)
wp(g3, ylim.all=.5)
```
...
Now I will fit the NBI, NBII, PIG and SICHEL distributions using `gamlss2`:


```{r}
#| error: true
library(gamlss2)
gpnbi  <- try(gamlss2(n_citation~re(random=~1|venue), data=da1, family=NBI))
gpnbii  <- gamlss2(n_citation~re(random=~1|venue), data=da1, family=NBII)
gppig  <- gamlss2(n_citation~re(random=~1|venue), data=da1, family=PIG)
gpsi  <- gamlss2(n_citation~re(random=~1|venue), data=da1, family=SICHEL)
```
...
```
Now fit covariates. This works, but with error messages:
