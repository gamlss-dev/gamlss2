...
In reality we do not know the distribution ${D}( )$  and also we do not know **which**  and **how** the variables   $({x}_{1i}, \ldots, {x}_{pi})$ effect the parameters $\theta_{1i}, \ldots, \theta_{ki}$. So the model selection in a distributional regression model should;
...
-  **step 0**: Defining a set of appropriate distributions for the response ${D_J()}$ for $j=1,\ldots, J,$ see section @sec-theranseoftherespons how to do that, 
